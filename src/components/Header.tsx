    ...
    <header className={[
      ...
      <div className="container flex h-[78px] items-center justify-between">
        ...
        <nav className="hidden items-center gap-7 lg:flex" aria-label="Primary navigation">
          {links.map(([label, href]) => <Link key={href} href={href} className="text-sm font-bold text-white/95 transition-colors hover:text-[#d4af37]">{label}</Link>)}
      ...
      {open && <>
        ...
        <aside className="fixed left-0 top-0 z-[60] h-screen w-[min(86vw,360px)] bg-[#0a192f] shadow-2xl lg:hidden">
          <div className="flex h-[78px] items-center justify-between border-b border-white/10 px-5"><Logo mobile /><button type="button" aria-label="Close navigation" onClick={() => setOpen(false)} className="rounded-lg p-2 text-white"><X size={22} /></button></div>
          <nav className="px-5 py-4" aria-label="Mobile navigation">
            {links.map(([label, href]) => <Link onClick={() => setOpen(false)} key={href} href={href} className="flex items-center justify-between border-b border-white/10 py-4 font-bold text-white">{label}<ChevronRight className="text-[#d4af37]" size={18} /></Link>)}
